  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-600">
        {config.animation === 'playful' && 'Romantic & playful vibes'}
        {config.animation === 'elegant' && 'Elegant & emotional'}
        {config.animation === 'cute' && 'Cute & modern'}
        {config.animation === 'casual' && 'Casual & fun'}
      </p>
      {isSelected && (
        <motion.div
          initial={{ scale: 0 }}
...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen relative">
      ...
      <section className="relative z-10 pt-12 pb-8 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-sm text-gray-600">No login required • 100% free • Mobile-friendly</span>
          </motion.div>
        </div>
      </section>
            ...
            >
              ...
              >
                <motion.button
                  ...
                  className={`
                    btn-shine px-8 py-4 rounded-full font-semibold text-lg shadow-lg
                  ...
                  `}
                >
                  {selectedType ? 'Create Your Proposal ✨' : 'Select a proposal type above'}
                </motion.button>
              </Link>
            </motion.div>
          </motion.div>
      ...
      {/* How It Works */}
      <section className="relative z-10 bg-white/60 backdrop-blur-sm py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { emoji: '✏️', title: 'Create', desc: 'Choose your proposal type, add names, and write your heartfelt message' },
              { emoji: '🔗', title: 'Share', desc: 'Get a unique link to send via WhatsApp, Instagram, or any messaging app' },
              { emoji: '🎉', title: 'Celebrate', desc: 'Watch the magic happen when they open your proposal and respond' },
            ].map((step, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                <h3 className="font-display text-xl font-semibold text-gray-800 mb-2">{step.title}</h3>
      ...
      <footer className="relative z-10 py-8 px-4 text-center">
        <p className="text-sm text-gray-500 mb-2">
          💝 Made with love • Images auto-delete after 5 days to keep it free
        </p>
        <p className="text-xs text-gray-400">
          © 2024 Say It With Love. Free for everyone, forever.
        </p>
      </footer>
    </main>
  );
